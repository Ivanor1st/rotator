...
async function ccLoadBrowse() {
  try {
    ...
    [...provSet].sort().forEach(p => {
      const emoji = provEmojis[p] || '🔌';
      provSel.innerHTML += `<option value="${p}" ${p === curProv ? 'selected' : ''}>${emoji} ${p}</option>`;
    });
...
function ccFilterBrowse() {
  ...
  const groups = {};
  const groupOrder = ['profile', 'ollama_cloud', 'nvidia', 'openrouter', 'google', 'local', ...keyProviders];
  filtered.forEach(m => {
  ...
  const sortedGroups = [...new Set(Object.keys(groups))].sort((a, b) => {
    const ia = groupOrder.indexOf(a), ib = groupOrder.indexOf(b);
    const valA = ia === -1 ? 99 : ia;
    const valB = ib === -1 ? 99 : ib;
    return valA - valB;
  });
